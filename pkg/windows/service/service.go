...
func QueryPreShutdownInfo() (*SERVICE_PRESHUTDOWN_INFO, error) {
	...
	for {
		...
		err := windows.QueryServiceConfig2(svc.StatusHandle(), windows.SERVICE_CONFIG_PRESHUTDOWN_INFO, &b[0], n, &n)
		if err == nil {
			break
		}
...
func UpdatePreShutdownInfo(timeoutMilliSeconds uint32) error {
	// Set preshutdown info
	preshutdownInfo := SERVICE_PRESHUTDOWN_INFO{
		PreshutdownTimeout: timeoutMilliSeconds,
	}

	err := windows.ChangeServiceConfig2(svc.StatusHandle(), windows.SERVICE_CONFIG_PRESHUTDOWN_INFO, (*byte)(unsafe.Pointer(&preshutdownInfo)))
	if err != nil {
		return err
	}
...
// the handler will only be set after InitService is called, so we use this to indicate if it is a windows service
func GetServiceControlHandler() *Handler {
	return thehandler
}
	...
	for {
		...
		case c := <-r:
			...
			case svc.PreShutdown:
				klog.Infof("Service pre-shutdown")
