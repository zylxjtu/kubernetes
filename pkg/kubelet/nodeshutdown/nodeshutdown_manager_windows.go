...
func NewManager(conf *Config) (Manager, lifecycle.PodAdmitHandler) {
	...
	if service.GetServiceControlHandler() == nil {
		m := managerStub{}
		return m, m
	}
...
func (m *managerImpl) start() (chan struct{}, error) {
	...
	currentInhibitDelay, err := service.QueryPreShutdownInfo()
	if err != nil {
		return nil, err
	}

	if periodRequested := m.periodRequested().Milliseconds(); periodRequested > int64(currentInhibitDelay.PreshutdownTimeout) {
		err := service.UpdatePreShutdownInfo(uint32(periodRequested))
		if err != nil {
		...
		updatedInhibitDelay, err := service.QueryPreShutdownInfo()
		if err != nil {
			return nil, err
		}

		if periodRequested > int64(currentInhibitDelay.PreshutdownTimeout) {
			return nil, fmt.Errorf("node shutdown manager was unable to update preshutdown info to %v (ShutdownGracePeriod), current value of preshutdown info (%v) is less than requested ShutdownGracePeriod", periodRequested, updatedInhibitDelay)
		}
	}

	// when to check if it is windows service
