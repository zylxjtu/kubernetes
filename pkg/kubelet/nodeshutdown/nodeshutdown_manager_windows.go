...
// can we use common interface for both windows and linux?
...
func (m *managerImpl) start() (chan struct{}, error) {
	...
	currentInhibitDelay, err := service.QueryPreShutdownInfo(s.Handle)
	if err != nil {
		return nil, err
	}

	m.logger.V(1).Info("shutdown manager get current preshutdown info", "currentInhibitDelay", currentInhibitDelay.PreshutdownTimeout)
